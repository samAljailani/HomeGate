...
export type OAuthProviderResponseDto = components['schemas']['OAuthProviderResponseDto']
export type OAuthProviderPatchRequestDto = components['schemas']['OAuthProviderPatchRequestDto']
...
class OAuthProviderService {
    async getAllOAuthProviders(pagination?: PaginationRequestDto): Promise<OAuthProviderResponseDto[]> {
        const { data, error } = await apiClient.GET('/api/oauth-providers', { params: { query: pagination } })
        if (error) throw error
        return data
    }

    async updateOAuthProvider(id: number, body: OAuthProviderPatchRequestDto): Promise<OAuthProviderResponseDto> {
        const { data, error } = await apiClient.PATCH('/api/oauth-providers/{id}', {
            params: { path: { id } },
            body,
        })
        if (error) throw error
