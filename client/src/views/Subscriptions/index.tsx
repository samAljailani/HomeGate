...
export function Subscriptions() {
    const { subscriptions, isLoading, patchSubscription, reload } =
        useAccountPage()
    ...
    const referenced = subscriptions.filter(
        (s) => s.accountType === 'REFERENCED'
    )
        ...
        <div className="py-8 space-y-8">
            <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                    Manage your service subscriptions and account access.
                </p>
            </div>
            ...
            {isLoading ? (
                ...
                <>
                    {accounts.length === 0 ? (
                        <p className="py-8 text-center text-muted-foreground">
                            You have no subscription accounts yet.
                        </p>
                    ) : (
                        <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            {accounts.map((sub) => (
                                <li
                                    key={sub.id}
                                    className="rounded-lg border bg-card p-4 space-y-3 shadow-sm transition-shadow hover:shadow-md dark:shadow-white/10"
                                >
                    ...
                    {referenced.length > 0 && (
                        <section className="space-y-3">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-muted-foreground">
                                    Services included with your managed
                                    accounts. These are derived automatically
                                    and cannot be managed individually.
                                </p>
                            </div>
                            <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                {referenced.map((sub) => {
                                    const sourceServiceName =
                                        sub.derivedFromSubscriptionId
                                            ? serviceNameBySubscriptionId.get(
                                                  sub.derivedFromSubscriptionId
                                              )
                                            : null
                                    ...
                                    return (
                                        <li
                                            key={sub.id}
                                            className="rounded-lg border bg-card p-4 space-y-3 opacity-80 shadow-sm transition-shadow hover:shadow-md dark:shadow-white/10"
                                        >
                                            <div className="flex items-center justify-between">
                                                <p className="font-medium">
                                                    {sub.serviceName}
